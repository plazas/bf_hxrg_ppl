...
def read_3x3_into_dict(file="", flux=False, nframes=4):
    ...
    for line in f:
        ...
        for i in range(1, nframes+1):
            ...
            if flux:
                dict[s[0]][2].append(float(s[i+2*nframes]))
    f.close()
    ...
    for key in dict:
        ...
        if flux:
            dict[key][2] = np.array(dict[key][2])
...
def linear_fit(x, y, y_err):
    pfinal, covar = optimize.curve_fit(
        fitfunc, x, y, p0=pinit, sigma=y_err,  maxfev=100000)
...
ppl_center_dict = read_3x3_into_dict(
    ppl_dir_center + "/jay_metric.dat", flux=True, nframes=nframes)
#
...
# THIS IS ACTUALLY MEAN SIGNAL PER FRAME ( (sample + 1 + sample) / 2) in ELECTRONS
samples = list(range(1, len(ppl_center_dict['2'][0]) + 1))
...
for key in ppl_center_dict:
    ...
    plt.errorbar(simulations_flux[key], sim_center_nc_dict[key][0]/factor,
                 yerr=sim_center_nc_dict[key][1], fmt='b-o', markersize=5, label='sim+noise+NL')
    plt.errorbar(simulations_flux[key], sim_center_dict[key][0]/factor,
                 yerr=sim_center_dict[key][1], fmt='g--o', markersize=5, label='sim+noise+NL, NL corr.')
    plt.errorbar(simulations_flux[key], sim_nothing[key][0]/factor,
                 yerr=sim_nothing[key][1], fmt='r-o', markersize=5, label='sim+no noise')

    if key == '5':

        m, m_err = linear_fit(
            ppl_center_dict[key][2], ppl_center_dict[key][0], None)
        ...
        if inset == True:
            ...
            plt.errorbar(
                samples, sim_center_nc_dict[key][0], yerr=sim_center_nc_dict[key][1], fmt='b-o', markersize=3, label='')
            #plt.errorbar (samples,flats_120v100[key][0], yerr=flats_120v100[key][1], fmt='m-o', markersize=3, label='flat1 vs flat2')
            plt.errorbar(
                samples, sim_center_dict[key][0], yerr=sim_center_dict[key][1], fmt='g--o', markersize=3, label='')
            plt.errorbar(
                samples, sim_nothing[key][0], yerr=sim_nothing[key][1], fmt='r-o', markersize=3, label='')
    ...
    if key == '2':
        plt.legend(loc='upper right', fancybox=True, ncol=1,
                   numpoints=1, prop=prop, handlelength=3)
...
for key in ppl_center_dict:
    ax = fig.add_subplot(3, 3, int(key))
    plt.errorbar(ppl_center_dict[key][2], ppl_center_dict[key][0],
                 yerr=ppl_center_dict[key][1], fmt='k-o', markersize=5, label='PPL data')
...
# ,data_sim_nl_nc, data_sim_nl, data_sim_nothing] #, data_sim_bf]
data_vec = [data_ppl_center]


# ,'m','c' ]  #ppl, nl corrected, nl non corrected, noiseless sims
c = ['k', 'b', 'g', 'r']
# , 'sim+noise+NL', 'sim+noise+NL, NL corr.', 'sim+no noise'] #, 'flat fields', 'sim+BF']
label_vec = ['PPL data']
...
for i, data in enumerate(data_vec):
    ...
    plt.errorbar(x, size_final_mean, yerr=yerr, markersize=8,
                 fmt=c[i]+'-o', label=label_vec[i])
    plt.legend(loc='upper left', fancybox=True, ncol=1,
               numpoints=1, prop=prop, handlelength=3)
    # plt.xlim([0,5])
...
vec = [(fc, 'Fc', 'green'), (b1, "B, method 1", 'blue'), (c1, "c1", 'yellow'), (c2, "c2", 'magenta'), (m, "m",
                                                                                                       'cyan'), (b2, "B (1/e$^-$)", 'red'), (c2_flats, "$C_2$ flats", 'red'), (c2_spots, "$C_2$ spots", 'blue')]
...
if sc:
    ...
    mean_b, sigma_b, indices = eu.stat.sigma_clip(
        b, niter=10, nsig=sigma_cut, get_indices=True, verbose=True)

    b = b[indices]
    fc = fc[indices]

    print("mean b, std b, std err of mean, min, max after 3-sigma clipping: ",
          mean_b, sigma_b, sigma_b/np.sqrt(len(b)), np.min(b), np.max(b))

    mean_fc, sigma_fc, indices = eu.stat.sigma_clip(
        fc, niter=10, nsig=sigma_cut, get_indices=True, verbose=True)

    b = b[indices]
    fc = fc[indices]

    print("mean fc, std fc, std err of mean after 3-sigma clipping: ",
          mean_fc, sigma_fc, sigma_fc/np.sqrt(len(fc)))
...
label_string = "Mean: %g \n Std. Error of Mean: %g" % (
    np.mean(b), np.std(b)/np.sqrt(len(b)))
...
ax.errorbar(x, y, yerr=y_err, ecolor='b', fmt='b.-',
            markersize=9, label=label_string)
plt.legend(loc='upper right', fancybox=True, ncol=1,
           numpoints=1, prop=prop, handlelength=3)
#plt.ylim([-6e-6, 6e-6])
# plt.xlim([1000,7000])
...
if sc:
    ...
    mean_b2_sim, sigma_b2_sim, indices = eu.stat.sigma_clip(
        b2_sim, niter=10, nsig=sigma_cut, get_indices=True, verbose=True)
    b2_sim = b2_sim[indices]

    mean_b2, sigma_b2, indices = eu.stat.sigma_clip(
        b2, niter=10, nsig=sigma_cut, get_indices=True, verbose=True)
    b2 = b2[indices]
else:
...
fig = fig = plt.figure()
for i in range(1, 10):
    median_flux_flats = np.genfromtxt(
        ppl_dir_center+"/jay_median_flux_flats_pixel_%g.dat" % i)
    data_residuals_flats = np.genfromtxt(
        ppl_dir_center+"/jay_residual_pixel_%g_flat.dat" % i)
    ...
    for y in data_residuals_flats:
        ax.errorbar(median_flux_flats, y, yerr=None, ecolor='g',
                    fmt='g.', markersize=7, label='', alpha=0.1)
    ax.errorbar(median_flux_flats, mean_y, yerr=None, ecolor='r',
                fmt='r.-', markersize=11, label='', alpha=1.0)
    if i in [7, 8, 9]:
...
fig = fig = plt.figure()
for i in range(1, 10):
    median_flux_flats_absolute = np.genfromtxt(
        ppl_dir_center+"/jay_median_flux_flats_pixel_%g.dat" % i)
    data_residuals_flats_absolute = np.genfromtxt(
        ppl_dir_center+"/jay_residual_absolute_pixel_%g_flat.dat" % i)
    ...
    for y in data_residuals_flats_absolute:
        ax.errorbar(median_flux_flats_absolute, y, yerr=None,
                    ecolor='b', fmt='b.', markersize=7, label='', alpha=0.1)
    ax.errorbar(median_flux_flats_absolute, mean_y_absolute, yerr=None,
                ecolor='r', fmt='r.-', markersize=11, label='', alpha=1.0)
    if i in [7, 8, 9]:
        ax.set_xlabel('Average signal \n (e$^-$)', size=9.5)
    if i in [1, 4, 7]:
...
simulated_mean_spot_fluxes_data = np.genfromtxt(
    ppl_dir_center+"/jay_median_spot_fluxes.dat")
...
for key in residual_functions_dict:
    ...
    print("key, median_flux_flats, np.abs(mean_y)/100: ",
          key, median_flux_flats, np.abs(mean_y)/100)


def get_error_no_norm_ramp(pixel_number_string, end_sample=5):
    simulated_ramp = np.array(list(range(1, end_sample+1))) * \
        simulated_mean_spot_fluxes[pixel_number_string]*delta_t
    print("key, simulated ramp: ", key, simulated_ramp)
    error = simulated_ramp * \
        residual_functions_dict[pixel_number_string](simulated_ramp)
    print("error: ", error)
    vec = []
    for i in range(len(error)-1):
        print("pixel_number_string: ", pixel_number_string,
              i, (error[i+1]-error[i]) / delta_t)
...
for key in ppl_center_dict:
    ...
    plt.errorbar(ppl_center_dict[key][2], (1000*ppl_center_dict_NO_NORM[key][0]),
                 yerr=ppl_center_dict[key][1], fmt='k-o', markersize=5, label='PPL data')
    ...
    plt.errorbar(ppl_center_dict[key][2],   (error_NO_NORM[key]),
                 yerr=None, fmt='r-o', markersize=5, label='NL error')
...
for key in ppl_center_dict:
    ...
    if key == '5':
        ...
        m, m_err = linear_fit(
            samples, 1000*ppl_center_dict_NO_NORM[key][0]/factor, None)
        print(m, m_err)
    ...
    ax.set_yticklabels([float(y)
                        for y in ax.get_yticks()], visible=True, size=7)
...
surr = read_surrounding_into_dict(
    ppl_dir_center+"/jay_metric_surrounding.dat", nframes=nframes)

fig = plt.figure()
samples = list(range(1, nframes+1))

ax = fig.add_subplot(1, 1, 1)
ax.errorbar(samples, surr['1'][0]/factor, yerr=surr['1'][1],
            ecolor='b', fmt='b.-', markersize=12, label='', alpha=1.0)
ax.errorbar(samples, surr['2'][0]/factor, yerr=surr['1'][1],
            ecolor='r', fmt='r*-', markersize=12, label='', alpha=1.0)
...
data_corner1 = read_3x3_into_dict(
    ppl_dir_corner_r1+"/jay_metric.dat", flux=True, nframes=nframes_corner)
data_corner2 = read_3x3_into_dict(
    ppl_dir_corner_r2+"/jay_metric.dat", flux=True, nframes=nframes_corner)
data_corner3 = read_3x3_into_dict(
    ppl_dir_corner_r3+"/jay_metric.dat", flux=True, nframes=nframes_corner)
data_corner4 = read_3x3_into_dict(
    ppl_dir_corner_r4+"/jay_metric.dat", flux=True, nframes=nframes_corner)


mean_corner = {"1": [[], [], []], "2": [[], [], []],
               "3": [[], [], []], "4": [[], [], []]}

mean_corner["1"][0] = (data_corner1["5"][0]+data_corner1["6"]
                       [0]+data_corner1["8"][0]+data_corner1["9"][0])*0.25
mean_corner["1"][1] = np.sqrt(data_corner1["5"][1]**2+data_corner1["6"]
                              [1]**2+data_corner1["8"][1]**2+data_corner1["9"][1]**2)
mean_corner["1"][2] = (data_corner1["5"][2]+data_corner1["6"]
                       [2]+data_corner1["8"][2]+data_corner1["9"][2])*0.25


mean_corner["2"][0] = (data_corner2["2"][0]+data_corner2["3"]
                       [0]+data_corner2["5"][0]+data_corner2["6"][0])*0.25
mean_corner["2"][1] = np.sqrt(data_corner2["2"][1]**2+data_corner2["3"]
                              [1]**2+data_corner2["5"][1]**2+data_corner2["6"][1]**2)
mean_corner["2"][2] = (data_corner2["2"][2]+data_corner2["3"]
                       [2]+data_corner2["5"][2]+data_corner2["6"][2])*0.25


mean_corner["3"][0] = (data_corner3["4"][0]+data_corner3["5"]
                       [0]+data_corner3["7"][0]+data_corner3["8"][0])*0.25
mean_corner["3"][1] = np.sqrt(data_corner3["4"][1]**2+data_corner3["5"]
                              [1]**2+data_corner3["7"][1]**2+data_corner3["8"][1]**2)
mean_corner["3"][2] = (data_corner3["4"][2]+data_corner3["5"]
                       [2]+data_corner3["7"][2]+data_corner3["8"][2])*0.25


mean_corner["4"][0] = (data_corner4["1"][0]+data_corner4["2"]
                       [0]+data_corner4["4"][0]+data_corner4["5"][0])*0.25
mean_corner["4"][1] = np.sqrt(data_corner4["1"][1]**2+data_corner4["2"]
                              [1]**2+data_corner4["4"][1]**2+data_corner4["5"][1]**2)
mean_corner["4"][2] = (data_corner4["1"][2]+data_corner4["2"]
                       [2]+data_corner4["4"][2]+data_corner4["5"][2])*0.25


fig = plt.figure()
for key in mean_corner:
    ax = fig.add_subplot(2, 2, int(key))
    ax.errorbar(mean_corner[key][2], mean_corner[key][0]/factor, yerr=mean_corner[key]
                [1], ecolor='k', fmt='k.-', markersize=6, label='', alpha=1.0)
    # plt.ylim([-0.006,0.006])
    # if key in ['1', '2']:
