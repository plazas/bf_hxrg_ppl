...
def main(argv):
 ...
 detector_seed = 456		#Changes the device non-uniformity (don't change if simulating a single device)
 ...
 backflux = 1.0                 # mean background in e-/s/px (affects object and flat)
 ...
 NL2_mean = -7.76e-7#-1.e-6  		#mean 2nd order coefficient for quadratic nonlinearity model
 ...
 cd = galsim.cdmodel.PowerLawCD(1, 1.1e-7, 1.1e-7, 1.0e-7, 1.0e-7, 0.0, 0.0, 0.0) # V1: 5, V2:4, V3:2.5, V4: 1.5, V5: 1.2, V6:1.0, V7: 1.1, V8: 1.1 + shot noise + centroid < 0.1, V9: 1.1 + no shot noise + centroid < 0.1
 ...
 header_dict = {
 'DETMODE':'32chSUTR' ,'BASE':basename ,'RAMPLEN':ramplen ,'RAMP1ST':0 ,'RAMPLAST':ramplen-1 ,'FRAMTIME':1000*frametime
 ,'EXPTIME':(ramplen-1)*frametime*1000
 ,'FILMSTRP': 'False'
 ,'CHANNELS':n_channels
 ,'NEXP':nexp
 ,'COMMENT':''
 ...
 if channel_gain_scatter>0:
 	channel_gains *= np.random.normal(1., channel_gain_scatter ,size=n_channels)
 ...
 ###################
 ### MAKE IMAGES ###
 np.random.seed(seed=noise_seed)   ##Careful when parallelizing!  Seed may get copied to each process

 for imgtype in imgtypes:

    header['TYPE'] = imgtype

    if 	 imgtype=='OBJECT': 	charge_template = (background+exposed_img*flux)*QE
    elif imgtype=='FLAT':	charge_template = (background+flatflux)*QE
    elif imgtype=='BACKGROUND':	charge_template = (background)*QE
    elif imgtype=='DARK':   	charge_template = 0.
    else: raise Exception('Unknown image simulation type')
	...
	cum_charge = np.array([charge_template for i in range(header['RAMPLEN'])])
	cum_charge[0] = 0.
	...
	if shotnoise_on:  cum_charge = np.random.poisson(cum_charge).astype('float64')
	...
	#    obs=sample[481, 1953]
        ...
        for sample in cum_charge:
            ...
            #temp = sample
            # IPC
            temp = ndimage.filters.convolve (sample, K, mode='constant', cval=0.0)
        ...
        if readnoise>0:  cum_charge += np.random.normal(0,readnoise,size=cum_charge.shape)
