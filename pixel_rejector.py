...
def getBounds(data = None, center = None, thresh = 60,median = False):
    ...
    def gaussian_logL(sigma):
        ...
        logL_each =-(data[these]-mode)**2/sigma**2/2. - np.log(det) + np.log(norm)
...
def find_anomalies(data,center  = None, forgive = .25):
    ...
    exthresh,mask = determine_quality_threshold(data,bins,excess/excess_err, ncore,forgive=forgive)
...
def main(argv):
    ...
    excess =  np.abs(h*data.size - Lmean)
    excess_err = np.sqrt(Lmean)
    exthresh,bad_inds = determine_quality_threshold(data,bins,excess/excess_err, ncore)
