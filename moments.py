...
def wcentroid(image, xcoords, ycoords, sigma=None, tol=1.e-4, maxnits=100, info=False, warn=True):
    ...
    if xcoords.shape != image.shape:
        raise ValueError(
            'Shape of supplied XCOORDS does not match input image')
    if ycoords.shape != image.shape:
        raise ValueError(
            'Shape of supplied YCOORDS does not match input image')
...
def crapsky(image, nedge=3):
    ...
    sky = image[((abs(x) > (hwx - float(nedge))) +
                 (abs(y) > (hwy - float(nedge))))]
...
def measure_moments(image, sigma=None, xc_guess=None, yc_guess=None, info=False,
    ...
    if skysub is True:
        imss -= crapsky(image, nedge=3)  # Sky subtraction
    ...
    # Then get the weighted second moments
        ...
        if info == True:
            print('measure_moments: Ixx = %e Ixy = %e Iyy = %e' %
                  (Ixx, Ixy, Iyy))
...
def sim_cubify(file, stampsize):
    ...
    # shape is (ngrid ,stampsize ,ngrid*stampsize)
    cube = np.array(np.array_split(image, ngrid, axis=0))
    # shape is (ngrid ,ngrid ,stampsize ,stampsize)
    cube = np.array(np.array_split(cube, ngrid, axis=2))
    # shape is (ngrid**2 ,stampsize ,stampsize)
    cube = cube.reshape((-1, stampsize, stampsize))
