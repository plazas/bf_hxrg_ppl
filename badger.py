...
def quad_correct(pixel_series=None, bias = None, flat = None, ivar_series = None, time_series=None, nl_coeff=None):
    ...
    c = np.sum( (1 - 2 * nl_coeff * (pixel_series - bias) ) * t * t * ivar_series)
    ...
    real_roots,nreal = np.unique(roots[np.imag(roots) == 0],return_counts=True)    
    if np.sum(nreal) == 1:
...
def deriveNonlinearity(flatFiles = None, samples_per_ramp = None, ramps_per_file =None, times = None, time_axis = 0, doplot = True):
    ...
    for i,imFile in enumerate(flatFiles):
        ...
        for j,fRamp in enumerate(flatRamps):
            this_nl_coeff =  makeNonlinearityTable(ramp = fRamp, timeseries = times[i])
            nl_coeff_est.append(this_nl_coeff)
            if doplot == True:
                plt.imshow(this_nl_coeff,vmin=-3e-6,vmax=3e-6,origin='lower')
                plt.colorbar()
                plt.savefig('nl_coeff-'+str(index)+'.pdf',format='pdf')
                plt.clf()
            index = index+1
    # stack the nonlinearity coefficients together
    nl_coeff_final = np.sum(np.stack(nl_coeff_est),axis=0)*( 1./(ramps_per_file[0] * nFiles ) )
...
def makeFrameFromRamp(ramp = None,flat = None, ivar = None, exptime = None, nl_coeff = None, getLogL = False, ramp_start = 1,
    ...
    if nl_coeff == None:
        f    = np.sum( (ramp -  bias - dgrid)*tgrid * ivar,axis=0) / (np.sum((flat*tgrid)**2 * ivar,axis=0) )
...
def organize_ramps(files = None,  expr = '([0-9]){4}_([0-9]){4}'):
    ...
    nFiles = len(files)
    fileStr = np.empty(nFiles, dtype = [('fileName',object),('baseName',object),('mode',object),('ramp',np.int),('sample',np.int),('EXPTIME',np.float),('FRAMTIME',np.float)])
    ...
    for thisRamp in rampNumbers:
        ...
        for this in these:
            if 'Multiple' in this['mode']:
                ...
                thisRamp = makeRampFromStrip(image,n_sample_per_ramp = this['sample'])[0] # CDS only
...
def makeRampFromStrip(image_orig = None, n_sample_per_ramp = None, n_ramp_per_file = 1, time_axis = 0):
    ...
    frame_length = image_orig.shape[time_axis] / n_sample_per_ramp / n_ramp_per_file
...
def updateHeader(header, inputFiles = None, masterDark = None, masterFlat = None, imageType = None, pixScale = 18.0, maskdist = 274.5):
    ...
    if masterDark is not None:
        header.set('MSTRDARK',masterDark,before='COMMENT',comment='master dark used for this image.')
    if masterFlat is not None:
        header.set('MSTRFLAT',masterFlat,before='COMMENT',comment = 'master flat used for this image.')
    if inputFiles is not None:
        header.set('HISTORY','        ')
        header.set('HISTORY','This image was made from the following image files:')
    ...
    header.set('MASKDIST',maskdist,comment = 'spot spacing, where relevant',before='COMMENT')
    return header
...
def makeCorrectedFrame( files_science = None, files_dark = None, files_flat = None,
    ...
    if do_dark == True:
        if (files_dark is not None) and (master_dark is None):
            ...
            masterDark = combine(ramps = dark_ramps, exptimes = darkStr['EXPTIME'] , ramp_start = ramp_start)
    ...
    if do_flat == True:
        if (files_flat is not None) & (master_flat is None):
            ...
            masterFlat = combine(ramps = flat_ramps, exptimes = flatStr['EXPTIME'] ,dark = masterDark, ramp_start = ramp_start)
    ...
    for i,sciRamp in enumerate(scienceRamps):
        ...
        if outFileTemplate is not None:
            ...
            outFileName = outFileTemplate+'-'+baseName+'-'+str(sciStr[i]['ramp']).zfill(4)+'.fits'
...
def doplot(image, catalog, outfile = 'cat-overlay.pdf' ):
    ...
    ax.plot(catalog[keep]['X_IMAGE'],catalog[keep]['Y_IMAGE'],'+',ms=0.5,color='red')
